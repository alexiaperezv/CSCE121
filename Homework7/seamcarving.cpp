    ...
    if (loadImage(filename, image, width, height)) {
      ...
      while(width > targetWidth || height > targetHeight){
        if(width > targetWidth){
          int* minSeam = findMinVerticalSeam(image, width, height);
          removeVerticalSeam(image, width, height, minSeam);
          deleteSeam(minSeam);
          width--;
        }
        if(height > targetHeight){
          int* minSeam = findMinHorizontalSeam(image, width, height);
          removeHorizontalSeam(image, width, height, minSeam);
          deleteSeam(minSeam);
          height--;
        }
      }
